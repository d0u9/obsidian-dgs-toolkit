...
export interface DgsToolkitSettings {
...
export const DEFAULT_SETTINGS: DgsToolkitSettings = {
...
export class DgsToolkitSettingTab extends PluginSettingTab {
	...
	constructor(app: App, private readonly plugin: DgsToolkitPlugin) {
		super(app, plugin);
	}

	getSettingDefinitions(): SettingDefinitionItem<keyof DgsToolkitSettings>[] {
		return [
			{
				type: 'page',
				name: 'Colon blocks',
	...
	): SettingGroupItem<keyof DgsToolkitSettings>[] {
		const editing = view === 'editing';
		const key = <K extends keyof DgsToolkitSettings>(
			readingKey: K,
			editingKey: keyof DgsToolkitSettings,
		): keyof DgsToolkitSettings => editing ? editingKey : readingKey;
		return [
			{
				name: 'Enable',
	...
	private renderSectionNavigation(): void {
		...
		for (const section of [
			...
			const button = navigation.createEl('button', {
				cls: 'dgs-settings-navigation__item',
				text: section.label,
			});
	...
	private renderContainerSettings(): void {
		...
		const blockList = this.containerEl.createDiv({ cls: 'dgs-colon-block-list' });
		for (const block of SUPPORTED_BLOCKS) {
			const item = blockList.createDiv({ cls: 'dgs-colon-block-list__item' });
	...
	private renderTypographySettings(): void {
		...
		const viewNavigation = this.containerEl.createDiv({
			cls: 'dgs-settings-navigation dgs-settings-navigation--secondary',
		});
		for (const view of [
	...
	private renderTypographyPreview(): void {
		const preview = this.containerEl.createDiv({ cls: 'dgs-typography-preview' });
		preview.createDiv({
			cls: 'dgs-typography-preview__label',
			text: this.activeTypographyView === 'reading' ? 'Reading view preview' : 'Editing view preview',
		});
		const sample = preview.createDiv({ cls: 'dgs-typography-preview__sample' });
		sample.createDiv({ cls: 'dgs-typography-preview__heading', text: PREVIEW_HEADING });
	...
	private applyTypographyPreview(): void {
		...
		applyVariables(
			...
			typographyVariables(this.plugin.settings, this.activeTypographyView, '--dgs-'),
		);
	}
	...
	private addFontFamilySetting(key: 'fontFamily' | 'editingFontFamily'): void {
		new Setting(this.containerEl)
			...
			.addText((text) => {
				text.inputEl.addClass('dgs-typography-font');
				text
	...
	private addTypographyControls(
		...
		setting.addText((text) => {
			...
			text.inputEl.addClass('dgs-typography-number');
...
type TypographyNumberKey = Exclude<keyof DgsToolkitSettings,
