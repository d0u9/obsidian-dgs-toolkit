...
const ADJACENCY_CLASSES = ['dgs-before-imgcap', 'dgs-before-compnote'] as const;
const PREVIEW_CHROME = '.markdown-preview-pusher, .mod-header, .mod-footer';
const LIST_DELIMITER_CLASS = 'dgs-list-delimiter';
...
const SEGMENT_CLASS = 'dgs-segment';
const SEGMENT_CLASSES = [
	...SEGMENT_TYPES.flatMap((type) => [
		`dgs-${type}-segment`,
		`dgs-${type}-segment--first`,
		`dgs-${type}-segment--last`,
		`dgs-${type}-delimiter`,
	]),
...
function applyAttributes(container: HTMLElement, opening: OpeningDelimiter): void {
	for (const name of opening.classes) container.addClass(`dgs-x-${name}`);
...
function clearUnclosed(element: HTMLElement): void {
	element.removeClass('dgs-unclosed-delimiter');
...
function markCrossSectionImgcaps(root: HTMLElement): void {
	...
	for (let index = 0; index < markers.length; index += 1) {
		...
		for (const segment of segments) {
			segment.addClasses([SEGMENT_CLASS, 'dgs-imgcap-segment']);
		}
		claimDelimiter(opener, 'dgs-imgcap-delimiter');
		claimDelimiter(closer, 'dgs-imgcap-delimiter');
		...
		bounds[0]?.addClass('dgs-imgcap-segment--first');
		bounds[bounds.length - 1]?.addClass('dgs-imgcap-segment--last');
		const preceding = blocks[first - 1];
		if (preceding && !preceding.matches(PREVIEW_CHROME)) {
			preceding.addClass('dgs-before-imgcap');
		}
		index = closingIndex;
	}
}
...
function buildContainers(root: HTMLElement, defaultType: string): void {
	...
	for (let index = 0; index < children.length; index += 1) {
		...
		if (!closer) {
			...
			opener.addClass('dgs-unclosed-delimiter');
		...
		if (opening.type === 'imgcap') {
			precedingBlock?.addClass('dgs-before-imgcap');
		} else if (opening.type === 'compnote' && precedingBlock?.tagName === 'HR') {
			precedingBlock.addClass('dgs-before-compnote');
		}
...
export function restoreCustomContainers(root: HTMLElement): void {
	clearAdjacencyClasses(root);
	root.querySelectorAll<HTMLElement>('.dgs-unclosed-delimiter').forEach(
		(element) => clearUnclosed(element),
	);
	unwrapListDelimiters(root);
	for (const container of Array.from(
		root.querySelectorAll<HTMLElement>('.dgs-custom-container'),
	)) {
		...
		const title = container.querySelector<HTMLElement>(
			':scope > .dgs-custom-container__title',
		)?.textContent;
		const content = container.querySelector<HTMLElement>(
			':scope > .dgs-custom-container__content',
		);
