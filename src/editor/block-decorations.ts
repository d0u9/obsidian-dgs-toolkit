 ...
 * with it.
 */
export function colonBlockEditorExtension(plugin: DgsToolkitPlugin): Extension {
...
function buildDecorations(state: EditorState, plugin: DgsToolkitPlugin): DecorationSet {
...
export function markLines(document: Text, defaultType: string): Map<number, LineMark> {
	...
	for (let number = 1; number <= document.lines; number += 1) {
		...
		if (closing && current && (closing[1] ?? '').length >= current.fence) {
			markInside(marks, line.from, open);
			addClass(marks, line.from, 'dgs-cm-delimiter');
			addClass(marks, line.from, 'dgs-cm-block--last');
			open.pop();
		...
		if (opening) {
			...
			if (KNOWN_TYPE.test(type)) {
				...
				addClass(marks, line.from, `dgs-cm-block--${type}`);
				addClass(marks, line.from, 'dgs-cm-block');
				addClass(marks, line.from, 'dgs-cm-delimiter');
				addClass(marks, line.from, 'dgs-cm-block--first');
				continue;
			}
		}
		...
		if (inside?.type === 'poem' && text.trim() === '') {
			addClass(marks, line.from, 'dgs-cm-block--poem-break');
		}
	}
	...
	for (const block of open) {
		addClass(marks, block.openedAt, 'dgs-cm-unclosed');
...
function markCjkVerse(
	...
	for (let number = block.startLine; number <= endLine; number += 1) {
		addClass(marks, document.line(number).from, 'dgs-cm-block--poem-cjk');
	}
}
...
function markInside(marks: Map<number, LineMark>, from: number, open: OpenBlock[]): void {
	const current = open[open.length - 1];
	if (!current) return;
	addClass(marks, from, 'dgs-cm-block');
	addClass(marks, from, `dgs-cm-block--${current.type}`);
	if (open.length > 1) addClass(marks, from, 'dgs-cm-block--nested');
}
