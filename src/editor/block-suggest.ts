...
export class ColonBlockSuggest extends EditorSuggest<BlockType> {
	...
	renderSuggestion(block: BlockType, element: HTMLElement): void {
		element.addClass('dgs-block-suggestion');
		element.createDiv({ cls: 'dgs-block-suggestion__type', text: block.type });
		element.createDiv({ cls: 'dgs-block-suggestion__desc', text: block.description });
	}
