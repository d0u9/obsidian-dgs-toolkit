...
export async function chooseAndPublishFolder(plugin: DgsToolkitPlugin): Promise<void> {
	if (publishInProgress) {
		new Notice('A folder is already being published.');
		return;
	}
	if (!plugin.settings.enableFolderPublishing) {
		new Notice('Enable folder publishing in DGS toolkit settings first.');
		return;
	}
	...
	if (!sourceSetting || !targetSetting) {
		new Notice('Set both publishing folders in DGS toolkit settings first.');
		return;
	}
...
async function publishFolder(source: string, target: string, folder: string): Promise<void> {
	...
	const staging = pathModule.join(parent, `.${pathModule.basename(target)}.dgs-copy-${suffix}`);
	const backup = pathModule.join(parent, `.${pathModule.basename(target)}.dgs-backup-${suffix}`);
...
async function removeStaleWorkFolders(
	...
	const prefixes = [`.${basename}.dgs-copy-`, `.${basename}.dgs-backup-`];
	let entries;
	try {
