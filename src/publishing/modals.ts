...
export class ConfirmPublishingModal extends Modal {
	...
	constructor(
		...
		super(app);
		this.modalEl.addClass('dgs-publishing-modal');
		...
		this.scope.register([], 'Escape', () => {
			if (this.contentEl.hasClass('dgs-publishing-content--diff')) {
				this.renderSummary();
				return false;
			}
			this.close();
	...
	private async runComparison(): Promise<void> {
		...
		if (this.contentEl.hasClass('dgs-publishing-content--summary')) this.renderSummary();
	}
	...
	private renderSummary(comparisonError?: string): void {
		...
		this.contentEl.empty();
		this.contentEl.removeClass('dgs-publishing-content--diff');
		this.contentEl.addClass('dgs-publishing-content--summary');
		this.contentEl.createEl('p', { text: DESTINATION_DESCRIPTIONS[this.options.targetKind] });

		const paths = this.contentEl.createDiv({ cls: 'dgs-publishing-preview' });
	...
	private renderChanges(comparisonError?: string): void {
		const section = this.contentEl.createDiv({ cls: 'dgs-publishing-changes' });
		if (comparisonError) {
			section.createDiv({
				cls: 'dgs-publishing-changes__note',
				text: `Could not compare the folders: ${comparisonError}`,
			});
			return;
		}
		if (!this.comparison) {
			section.createDiv({ cls: 'dgs-publishing-changes__note', text: 'Comparing folders…' });
			return;
		}
		...
		section.createDiv({
			cls: 'dgs-publishing-changes__summary',
			text: counts
		...
		const header = section.createDiv({ cls: 'dgs-publishing-changes__header' });
		...
		header.createSpan({
			cls: 'dgs-publishing-changes__count',
			text: `${selectedCount} of ${changes.length} changes selected`,
		});

		const list = section.createDiv({ cls: 'dgs-publishing-changes__list' });
		for (const change of changes) {
			const row = list.createDiv({
				cls: `dgs-publishing-change dgs-publishing-change--${change.status}`,
			});
			...
			const open = row.createEl('button', { cls: 'dgs-publishing-change__open' });
			open.createSpan({
				cls: 'dgs-publishing-change__badge',
				text: CHANGE_LABELS[change.status],
			});
			open.createSpan({ cls: 'dgs-publishing-change__path', text: change.path });
			if (this.pulledPaths.has(change.path) && this.canPull(change)) {
				open.createSpan({
					cls: 'dgs-publishing-change__pull',
					text: '↩ vault',
	...
	private renderDiffPaths(change: FolderChange): void {
		...
		const block = this.contentEl.createDiv({ cls: 'dgs-publishing-preview' });
		block.createDiv({
	...
	private renderSideChooser(change: FolderChange, onPick: () => void): () => void {
		const chooser = this.contentEl.createDiv({ cls: 'dgs-publishing-choice' });
		const options = SIDE_CHOICES[change.status];
		const buttons = ([
			['destination', options.destination],
			['source', options.source],
		] as const).map(([side, option]) => {
			const button = chooser.createEl('button', { cls: `dgs-publishing-choice__side is-${side}` });
			button.createDiv({ cls: 'dgs-publishing-choice__title', text: option.title });
			button.createDiv({ cls: 'dgs-publishing-choice__desc', text: option.description });
			return { side, button };
		});
	...
	private renderImageDiff(
		...
		const panes = body.createDiv({ cls: 'dgs-publishing-image' });
		...
		const delta = body.createDiv({ cls: 'dgs-publishing-image__delta' });
	...
	private renderImagePane(
		...
		const pane = parent.createDiv({ cls: 'dgs-publishing-image__pane' });
		pane.createDiv({ cls: 'dgs-publishing-image__label', text: label });
		if (!image) {
			pane.createDiv({ cls: 'dgs-publishing-image__missing', text: 'Not present' });
			return Promise.resolve(null);
		}

		const url = URL.createObjectURL(new Blob([image.bytes], { type: image.mime }));
		this.objectUrls.push(url);
		const element = pane.createEl('img', { cls: 'dgs-publishing-image__preview' });
		element.src = url;
		const meta = pane.createDiv({ cls: 'dgs-publishing-image__meta', text: formatBytes(image.byteLength) });
	...
	private async renderDiff(change: FolderChange): Promise<void> {
		...
		this.contentEl.empty();
		this.contentEl.removeClass('dgs-publishing-content--summary');
		this.contentEl.addClass('dgs-publishing-content--diff');
		...
		refreshPull();
		const body = this.contentEl.createDiv({ cls: 'dgs-publishing-diff' });
		body.createDiv({ cls: 'dgs-publishing-diff__note', text: 'Loading…' });
		...
		try {
			...
			body.empty();
			body.createDiv({
				cls: 'dgs-publishing-diff__note',
				text: `Could not read the file: ${errorMessage(error)}`,
			});
			return;
		}
		// Bail out if the user went back while the file was being read.
		if (!this.contentEl.hasClass('dgs-publishing-content--diff')) return;

		body.empty();
		if (diff.kind === 'image') {
			body.addClass('dgs-publishing-diff--image');
			this.renderImageDiff(body, diff.before, diff.after);
			return;
		}
		if (diff.kind !== 'text') {
			body.createDiv({ cls: 'dgs-publishing-diff__note', text: NON_TEXT_NOTES[diff.kind] });
			return;
		}
		body.addClass('dgs-publishing-diff--text');
		const text = diff;
	...
	private paintTextDiff(
		...
		body.empty();
		const header = body.createDiv({ cls: 'dgs-publishing-diff__header' });
		...
		for (const row of diff.rows) {
			if (row.kind === 'gap') {
				body.createDiv({ cls: 'dgs-publishing-diff__gap', text: `⋯ ${row.text ?? ''}` });
				continue;
			}
			...
			const side = row.kind === 'change'
				...
				: '';
			const element = body.createDiv({ cls: `dgs-publishing-diff__row is-${row.kind}${side}` });
	...
	private renderHunkChooser(
		...
		const bar = body.createDiv({ cls: 'dgs-publishing-hunk' });
		bar.createSpan({
			cls: 'dgs-publishing-hunk__label',
			text: `Change ${hunk + 1} of ${hunkCount}`,
		});
		const actions = bar.createDiv({ cls: 'dgs-publishing-hunk__actions' });
		for (const [side, label] of [['destination', 'Use left'], ['source', 'Use right']] as const) {
			const button = actions.createEl('button', {
				cls: 'dgs-publishing-hunk__button',
				text: label,
			});
			const selected = kept === (side === 'destination');
			button.toggleClass('is-selected', selected);
...
function renderDiffCell(
	...
	const side = row.createDiv({ cls: `dgs-publishing-diff__cell is-${cell ? tone : 'empty'}` });
	side.createSpan({ cls: 'dgs-publishing-diff__gutter', text: cell ? String(cell.line) : '' });
	side.createSpan({ cls: 'dgs-publishing-diff__text', text: cell?.text ?? '' });
}
