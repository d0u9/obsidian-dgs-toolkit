...
export class ConfirmPublishingModal extends Modal {
	...
	private async renderDiff(change: FolderChange): Promise<void> {
		...
		for (const line of diff.lines) {
			const row = body.createDiv({ cls: `dgs-publishing-diff__line is-${line.kind}` });
			if (line.kind === 'gap') {
				row.createSpan({ cls: 'dgs-publishing-diff__gap', text: `⋯ ${line.text}` });
				continue;
			}
			row.createSpan({
				cls: 'dgs-publishing-diff__gutter',
			...
			row.createSpan({
				cls: 'dgs-publishing-diff__sign',
				text: line.kind === 'add' ? '+' : line.kind === 'remove' ? '−' : ' ',
			});
			row.createSpan({ cls: 'dgs-publishing-diff__text', text: line.text });
		}
	}
}
