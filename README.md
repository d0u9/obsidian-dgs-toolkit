...
Repository: [d0u9/obsidian-dgs-toolkit](https://github.com/d0u9/obsidian-dgs-toolkit)
...
`dgs-x-<name>`, and every other pair becomes a `data-<key>` attribute to style
...
`DGS_PLUGIN_OUTDIR` in the environment or in an untracked `.env`:

```bash
DGS_PLUGIN_OUTDIR=/path/to/vault/.obsidian/plugins/dgs-toolkit
```
...
installed plugin ID remains `dgs-toolkit`, and the user-facing name remains
**DGS Toolkit**.
